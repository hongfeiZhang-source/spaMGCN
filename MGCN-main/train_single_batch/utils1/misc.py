...
def print_results( nmi_result, ari_result, f1_result, args,state):
    print(state)
    print("name: {}".format(args.name))
    print("NMI : {:.4f}".format(max(nmi_result)))

    print("ARI : {:.4f}".format(ari_result[np.where(nmi_result == np.max(nmi_result))[0][0]]))
    print("AMI  : {:.4f}".format(f1_result[np.where(nmi_result == np.max(nmi_result))[0][0]]))
...
import torch  
import numpy as np  
from sklearn.metrics import adjusted_mutual_info_score, normalized_mutual_info_score as nmi_score, adjusted_rand_score as ari_score, \
    homogeneity_score, completeness_score, v_measure_score  

# def cluster_acc(y_true, y_pred):  
#     """  
#     Calculate clustering accuracy.  This requires a dense assignment.  

#     This implementation is taken from: https://github.com/XifengGuo/DEC-pytorch/blob/master/main.py  
#     """  
...
#     return sum([w[i, j] for i, j in ind]) * 1.0 / y_true.size, 0  #  返回 accuracy 和 f1 (这里 f1 初始化为0，需要根据实际情况计算)  
